...
std::pair<Move,int> AMAFPlayer::split(std::vector<Move>::const_iterator pStart, std::vector<Move>::const_iterator pEnd, const Board currentState, const Board::STATE pGoal){
	...
	for(;pStart != pEnd; pStart++){
		...
		int x = MCPlayer::simulation(pGoal, lNewBoard, SIMULATIONS_PER_SPLIT / lSize);
		if(x > lMaxValue){
			lMaxValue = x;
			lMaxMove = *pStart;
		}
